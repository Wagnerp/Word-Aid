...
void		CColorPickerCB::SetSelectedColorValue( COLORREF crClr )
{
	...
	// If the colour is not found add it as the custom colour 
	// and select it
	if(!bFound)
	{
		// TODO : Add string to String table
		int nIndex = AddColor(_T("Custom"), crClr);
		SetCurSel(nIndex);
	}
}
...
int			CColorPickerCB::AddColor( PCSTR cpName, COLORREF crColor )
{
	...
	// If string is already in the combo then remove and add new one
...
void CColorPickerCB::MeasureItem(LPMEASUREITEMSTRUCT)
{
}

int CColorPickerCB::CompareItem(LPCOMPAREITEMSTRUCT)
{
	return 0;
}
